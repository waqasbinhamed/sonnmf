...
def cyclic(H, M, W, W_update_iters, gamma, lam, cyc):
    m, rank = W.shape
    for it in range(W_update_iters):
        for j in range(rank):
            hj = H[j:j + 1, :]
            ...
            prox_pen = np.median(np.hstack((w_bar + (gamma / (hj_norm_sq + EPS)), np.zeros((m, 1)), w_bar)), axis=1,
                                 keepdims=True)
            prox_w_sum = np.sum(
                np.array([prox(lam / (hj_norm_sq + EPS), W[:, ((j + (k - j) % rank) % rank):((j + (k - j) % rank) % rank) + 1], w_bar) for k in range(j + 1, j + 1 + cyc)]), axis=0)
